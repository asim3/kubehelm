...
from k8s.manifests.api import APIFunctionsMixin

from kubernetes.client import CoreV1Api, ApiextensionsV1Api, RbacAuthorizationV1Api


class TestAPIFunctionsMixin(TestCase):
    pod = {
        "apiVersion": "v1",
        "kind": "Pod",
        "metadata": {
    ...
    def test_pod_apply(self):
        api_func = APIFunctionsMixin().get_api_function("create", self.pod, testing=True)
        expected = getattr(CoreV1Api, "create_namespaced_pod")
        self.assertEqual(api_func, expected)

    def test_pod_update(self):
        api_func = APIFunctionsMixin().get_api_function("patch", self.pod, testing=True)
        expected = getattr(CoreV1Api, "patch_namespaced_pod")
        self.assertEqual(api_func, expected)

    def test_pod_delete(self):
        api_func = APIFunctionsMixin().get_api_function("delete", self.pod, testing=True)
    ...
    def test_rbac_apply(self):
        api_func = APIFunctionsMixin().get_api_function(
    ...
    def test_rbac_update(self):
        api_func = APIFunctionsMixin().get_api_function("patch", self.rbac, testing=True)
        expected = getattr(RbacAuthorizationV1Api,
    ...
    def test_ext_apply(self):
        api_func = APIFunctionsMixin().get_api_function("create", self.ext, testing=True)
        expected = getattr(ApiextensionsV1Api,
                           "create_custom_resource_definition")
        self.assertEqual(api_func, expected)

    def test_ext_update(self):
        api_func = APIFunctionsMixin().get_api_function("patch", self.ext, testing=True)
        expected = getattr(ApiextensionsV1Api,
                           "patch_custom_resource_definition")
        self.assertEqual(api_func, expected)

    def test_ext_delete(self):
        api_func = APIFunctionsMixin().get_api_function("delete", self.ext, testing=True)
        expected = getattr(ApiextensionsV1Api,
